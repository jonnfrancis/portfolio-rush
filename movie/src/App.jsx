...
const App = () => {
  ...
  const fetchMovies = async (query = '') => {
    ...
    setErrorMessage('');
    try {
      const endpoint = query ? `${API_BASE_URL}/search/movie?query=${encodeURIComponent(query)}&sort_by=popularity.desc&include_adult=false`
    ...
    <main>
      ...
      <div className="wrapper">
        ...
        <section className="all-movies">
          ...
          {isLoading ? (<div className="spinner"></div>) : errorMessage ? ( <p className="text-red-500">{errorMessage}</p> ) : (
            <ul>
              {movieList.map((movie) => (
